...
public class LiveEntity : GeoGroObject
{
    ...
    void UpdateView()
    {
        ...
        Quaternion prevLocalRot =
            tiltOffset * prevRot * Quaternion.Inverse(transform.rotation);
        tiltOffset = Quaternion.identity;
        //傾きをイージングする為に一旦カメラを傾ける
        cameraTiltRot = cameraTiltRot * prevLocalRot;
    ...
    public void AddTiltOffset(Quaternion quaternion)
    {
        if (updating)
        {
            tiltOffset *= quaternion;
        }
    }
